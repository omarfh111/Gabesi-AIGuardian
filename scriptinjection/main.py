...
def clean_text(text: str) -> str:
    # 2. remove references / bibliography section (stop after it)
    text = re.split(r'references|bibliography|bibliographie', text, flags=re.IGNORECASE)[0]
    
    # 2. remove page numbers floating inside text
    text = re.sub(r'\n\d+\n', '\n', text)
    
    # Minimize repeated whitespaces and newlines
    text = re.sub(r'\s+', ' ', text)
    
    # Remove basic headers/footers numbers commonly left by PDF conversions
...
def extract_keywords(text: str) -> List[str]:
    # Bonus: Boost Retrieval with basic keyword extraction (top 5 frequent unique words >= 6 chars)
...
def extract_section_title(text: str) -> str:
    # Bonus: Attempt to extract an uppercase section title at the start
...
def process_document_pages(file_path: Path) -> List[str]:
    # 1. FIX: Streaming, return list of pages instead of full text
...
def load_documents(base_path: str) -> List[Dict[str, Any]]:
    docs = []
    ...
    for specialty_folder in base_dir.iterdir():
        if not specialty_folder.is_dir():
            continue
            
        import unidecode
        raw_specialty = unidecode.unidecode(specialty_folder.name).lower().replace(" ", "_")
        
        for file_path in specialty_folder.rglob("*"):
            if not file_path.is_file():
                continue
                
            start_time = time.time()
            # Returns list of pages for PDF
...
def generate_chunks(raw_docs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for doc in tqdm(raw_docs, desc="Extracting, Cleaning & Chunking"):
        # 7. BEST PRACTICE: PDF -> PAGE -> CLEAN -> CHUNK
...
# 4. FIX: Retry Mechanism for embeddings
...
def upload_to_qdrant(structured_chunks: List[Dict[str, Any]]):
    ...
    # 5. FIX: Dynamic Vector Size Detection
...
def main():
    ...
    base_proj_dir = os.path.dirname(os.path.dirname(__file__))
    data_path = os.path.join(base_proj_dir, "data", "datamed")
    
    print("\n=== MEDICAL RAG PIPELINE V2 ===")
    
    docs = load_documents(data_path)
    print(f"\n[Stats] Extracted {len(docs)} documents (processed per page for memory scaling).")
    ...
    print(f"[Stats] Created {len(chunks)} cleaned, semantic chunks across {len(set(d['specialty'] for d in docs))} specialties.")
    
    upload_to_qdrant(chunks)
    
    print("\n=== PIPELINE SUCCESS ===")
    print("All collections are provisioned and populated.")
