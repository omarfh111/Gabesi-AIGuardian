...
import { fetchUserData, runParallelAnalysis, healthCheck } from "./api";
import ProfileForm from "./ProfileForm";
import Dashboard from "./Dashboard";
import "./App.css";

function App() {
  const [page, setPage] = useState("form"); // "form" par défaut au démarrage
  ...
  if (page === "form") {
    return <ProfileForm onBack={() => setPage("home")} onSubmit={handleFormSubmit} />;
  }
          ...
          <div className="profile-grid">
            <div className="profile-stat">
              ...
              <div className="profile-stat__value profile-stat__value--green">
                {userData.identite?.["salaire_tnd_accumulé"]} TND
              </div>
            </div>
            <div className="profile-stat">
...
function formatAnalysis(text) {
  if (!text) return <span style={{ color: "var(--text-muted)" }}>Aucune analyse disponible.</span>;

  return text.split("\n").map((line, i) => {
    if (line.startsWith("###")) {
      return (
        <h4 key={i} style={{ color: "var(--text-primary)", margin: "16px 0 8px", fontSize: "0.95rem", fontWeight: 700 }}>
          {line.replace(/^###\s*/, "")}
        </h4>
      );
    }
    if (line.startsWith("##")) {
      return (
        <h3 key={i} style={{ color: "var(--text-primary)", margin: "18px 0 8px", fontSize: "1rem", fontWeight: 700 }}>
          {line.replace(/^##\s*/, "")}
        </h3>
      );
    }
    ...
    if (numMatch) {
      return (
        <div key={i} style={{ paddingLeft: "20px", position: "relative", marginBottom: "4px" }}>
          <span style={{ position: "absolute", left: 0, color: "var(--accent-cyan)", fontWeight: 700, fontSize: "0.85rem" }}>
            {numMatch[1]}.
          </span>
