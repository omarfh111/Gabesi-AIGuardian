...
from app.medical_triage.models.intake import PatientIntake
from app.medical_triage.models.analysis import TriageAnalysis
from app.medical_triage.services.rag_service import RAGService
...
class TriageAnalysisService:
    ...
    def analyze(self, intake: PatientIntake) -> TriageAnalysis:
        ...
        user_prompt = (
            ...
            "Instructions:\n"
            "1. DIFFERENTIAL REASONING: Analyze if symptoms are 'Clustered/Pathological' (Specific to a domain) or 'Scattered/Common' (Vague symptoms like slight headache, fatigue, mild cough).\n"
            "2. OVER-SPECIALIZATION WARNING: Do NOT assign a high score to a specialist for non-specific symptoms just because a checkbox was selected. If symptoms lack focal red flags, favor the 'generalist'.\n"
            "3. Rank relevant specialties (MUST stick to this list: pneumologist, cardiologist, oncologist, neurologist, dermatologist, toxicologist, generalist).\n"
            "4. Summarize the case (Explicitly address industrial/workplace exposure if high-risk. Utilize 'duration' and 'progression' for clinical context).\n"
            "5. Estimate urgency (low | moderate | high | emergency).\n"
            "6. Suggest possible conditions (with confidence scores).\n"
            "7. ALWAYS include the disclaimer: 'This is a preliminary assessment...'\n\n"
