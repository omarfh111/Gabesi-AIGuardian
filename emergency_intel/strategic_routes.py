...
@strategic_bp.route('/chat', methods=['POST'])
@traceable(name="Strategic Agentic Chat")
def chat():
...
@strategic_bp.route('/upload', methods=['POST'])
def upload_data():
    ...
    try:
        if industry_uploaded:
            subprocess.run([sys.executable, "scripts/ingest_industry.py"], check=True)
            
        if fishing_uploaded:
            subprocess.run([sys.executable, "scripts/process_fishing.py"], check=True)
...
@strategic_bp.route('/analysis', methods=['GET'])
...
@strategic_bp.route('/files', methods=['GET'])
def get_saved_files():
...
@strategic_bp.route('/files/<file_type>/<filename>', methods=['DELETE'])
